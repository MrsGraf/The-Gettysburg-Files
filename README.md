# The Gettysburg Files — device-review baseline v67
...
The Final Recovery Protocol opens only after an intentional tap on the ready Master Recovery Key block. Fragment numbers are not displayed in the directory or inside the protocol; each fragment is revealed in its file-recovery sequence and can later be reopened from the restored file.

## v67 · Case File D reading flow and fifth device record
- Audited the automatic scroll/anchor behaviour throughout Case File D so newly revealed explanations remain readable before the next task.
- The D02 reference button now lands on `ANOTHER LAYER IN THE PASSAGE`; the Parallelism result stays beside its Verify button and no longer triggers a jump.
- The Antithesis analysis progression appears only after Antithesis verification.
- Birth-metaphor verification remains in place for the semantic-field explanation; finding `new birth of freedom` deliberately returns to `SCAN THE SPEECH AGAIN`, where `METAPHOR EXTENDED` appears.
- Birth-metaphor markings now use turquoise, while the closing `of/by/for the people` structure uses orange after verification.
- The D sequence now presents five visible stylistic-device records; Antithesis is marked as Record 03 within the continuous D02 workflow, Metaphor is Record 04, and the closing three-part formula is Record 05.
- Record 05 opens only after `EXAMINE THE NEXT PASSAGE`; its recovery sequence still requires a separate final confirmation.
