window.initCaseA=function(){
  ...
  map?.addEventListener('pointerup',event=>{
    ...
    // Coordinates are relative to the rendered image so the target scales reliably on tablets.
    const targetX=.546,targetY=.362;
    const dx=(x-targetX)/.04,dy=(y-targetY)/.045;
  ...
  setupAssignment({pool:'#casePool .note',slots:'.cat',verify:'verifyCaseFacts',retry:'a4retry',success:'a4fb',complete:'a4complete',key:'cat',recovery:{caseId:'A',fragment:'18',nextId:'B0',nextLabel:'CONTINUE TO NEXT FILE'}});
  ...
  function setupAssignment({pool,slots,verify,retry,success,complete,key='value',recovery=null}){
    ...
    verifyBtn?.addEventListener('click',()=>{
      ...
      document.getElementById(retry)?.classList.toggle('show',!ok);document.getElementById(success)?.classList.toggle('show',ok);if(complete)document.getElementById(complete)?.classList.toggle('show',ok);if(ok&&recovery)setTimeout(()=>showRecoveryOverlay(recovery),450);
    });
  }
};
