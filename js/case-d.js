window.initCaseD=function(){
  ...
  // Temporary v42 D review state: earlier device files are pre-recovered so the final D05 recovery
  // and the subsequent Master Key / epilogue sequence can be tested without redoing D01-D04.
  function applyDReviewRecoveredState(){
    // D01
    const d1Input=document.getElementById('d1DeviceInput');
    if(d1Input){d1Input.value='repetition';d1Input.disabled=true;}
    const d1DeviceButton=document.getElementById('verifyD1Device');
    markVerifyButton(d1DeviceButton);if(d1DeviceButton)d1DeviceButton.disabled=true;
    document.getElementById('d1DeviceRecovered')?.classList.add('show');
    const d1Analysis=document.getElementById('d1AnalysisBlock');if(d1Analysis)d1Analysis.hidden=false;
    document.getElementById('d1LayersRecovered')?.classList.add('show');
    const d1Verify=document.getElementById('verifyD1Layers');if(d1Verify){markVerifyButton(d1Verify);d1Verify.disabled=true;}
    document.querySelectorAll('.d1-example').forEach(x=>{x.classList.remove('active');x.classList.add('assigned');x.disabled=true;});
    document.querySelectorAll('.d1-layer-slot').forEach(slot=>{
      const note=document.querySelector(`.d1-example[data-layer="${slot.dataset.layer}"]`);
      slot.classList.add('filled');
      slot.textContent=`${slot.dataset.label} · ${note?.textContent||''}`;
      slot.dataset.assignedValue=note?.textContent||'';
      slot.dataset.assignedLayer=slot.dataset.layer;
      slot.disabled=true;
    });

    // D02
    const d2Button=document.getElementById('verifyD2Antithesis');
    markVerifyButton(d2Button);if(d2Button)d2Button.disabled=true;
    document.getElementById('d2AntithesisRecovered')?.classList.add('show');
    const d2Examples=document.getElementById('d2AnalysisExamples');if(d2Examples)d2Examples.hidden=false;
    const d2Input=document.getElementById('d2AntithesisInput');if(d2Input){d2Input.value='what we say here / what they did here';d2Input.disabled=true;}

    // D04
    document.querySelectorAll('#d4Speech .word-token').forEach(token=>{
      token.classList.toggle('selected',['brought forth','conceived','new birth'].includes(token.dataset.word));
      token.disabled=true;
    });
    const d4PatternButton=document.getElementById('verifyD4Pattern');
    markVerifyButton(d4PatternButton);if(d4PatternButton)d4PatternButton.disabled=true;
    document.getElementById('d4PatternRecovered')?.classList.add('show');
    const d4Device=document.getElementById('d4DeviceBlock');if(d4Device)d4Device.hidden=false;
    const d4DeviceButton=document.getElementById('verifyD4Device');
    markVerifyButton(d4DeviceButton);if(d4DeviceButton)d4DeviceButton.disabled=true;
    const d4Input=document.getElementById('d4DeviceInput');if(d4Input){d4Input.value='birth metaphor';d4Input.disabled=true;}
    const d4Function=document.getElementById('d4FunctionBlock');if(d4Function)d4Function.hidden=false;
    document.getElementById('d4FunctionRecovered')?.classList.add('show');
    const d4Briefing=document.getElementById('d4Briefing');if(d4Briefing)d4Briefing.hidden=false;
    document.querySelectorAll('#d4FunctionOptions .skill-option').forEach(x=>{x.classList.toggle('selected',x.dataset.correct==='1');x.disabled=true;});
  }

  if(GettysburgState.isTestMode())applyDReviewRecoveredState();
