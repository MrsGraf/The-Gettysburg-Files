window.initCaseD=function(){
  ...
  document.getElementById('verifyD1Device')?.addEventListener('click',()=>{
    ...
    const ok=v.includes('repetition')||closeEnough(v,'repetition');
    ...
    if(ok){const b=document.getElementById('d1AnalysisBlock');if(b)b.hidden=false;}
  });
  ...
  document.getElementById('verifyD2Antithesis')?.addEventListener('click',()=>{
    ...
    const livingDead=v.includes('living and dead')||v.includes('living dead');
    const correct=
      (v.includes('say')&&v.includes('did'))||
      (v.includes('word')&&v.includes('action'))||
      (v.includes('speaker')&&v.includes('soldier')&&(v.includes('word')||v.includes('action')))||
    ...
    if(correct){const b=document.getElementById('d2AnalysisExamples');if(b)b.hidden=false;}
  });
  ...
  document.getElementById('verifyD4Pattern')?.addEventListener('click',()=>{
    ...
    const ok=['brought forth','conceived','new birth'].every(x=>selected.includes(x));
    ...
    if(ok){const b=document.getElementById('d4DeviceBlock');if(b)b.hidden=false;}
  });

  document.getElementById('verifyD4Device')?.addEventListener('click',()=>{
    const v=norm(document.getElementById('d4DeviceInput')?.value);
    const person=v.includes('personification')||closeEnough(v,'personification');
    const ok=v.includes('metaphor')||closeEnough(v,'metaphor')||person;
    ...
    if(ok){const b=document.getElementById('d4FunctionBlock');if(b)b.hidden=false;}
  });
