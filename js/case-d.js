window.initCaseD=function(){
  ...
  function holdReadingPosition(callback){
    // New explanation blocks are inserted next to the current task. Do not force
    // repeated scroll restoration: it can cancel taps and produce visible jumps,
    // especially in iPad Safari.
    callback();
  }
  ...
  function d1SelectionState(){
    const words=[...document.querySelectorAll('.d1-selectable-word')];
    const selected=words.filter(word=>word.classList.contains('selected'));
    const repetition=words.filter(word=>word.dataset.repetitionAnswer==='true');
    const same=(a,b)=>a.length===b.length&&a.every(x=>b.includes(x));
    if(same(selected,repetition))return 'correct';
    const validSets=[
      ['what','we','say','here'],
      ['what','they','did','here']
    ].map(tokens=>words.filter(w=>tokens.includes(w.dataset.token||'')));
    if(validSets.some(set=>same(selected,set)))return 'valid-other';
    return 'retry';
  }
  document.getElementById('verifyD1Marking')?.addEventListener('click',()=>{
    const readingY=window.scrollY;
    const state=d1SelectionState();
    const ok=state==='correct';
    document.getElementById('d1MarkingRetry')?.classList.toggle('show',state==='retry');
    document.getElementById('d1MarkingOtherValid')?.classList.toggle('show',state==='valid-other');
  ...
  // D02 · reference box and context workflow are visible in one continuous view.
  ...
  document.getElementById('verifyD2Antithesis')?.addEventListener('click',()=>{
    const readingY=window.scrollY;
    const leftWord=norm(contrastSelections.a?.textContent||'');
    const rightWord=norm(contrastSelections.b?.textContent||'');
    const ok=(leftWord==='say'&&rightWord==='did')||(leftWord==='we'&&rightWord==='they');
    ...
    if(screen){screen.dataset.progress='50';if(typeof updateProgress==='function')updateProgress(screen);}
  ...
  document.getElementById('verifyD3Image')?.addEventListener('click',()=>{
    ...
    holdReadingPosition(()=>{const returnStage=document.getElementById('d3ReturnStage');if(returnStage)returnStage.hidden=false;},readingY);
  });
  document.getElementById('openD3Scan')?.addEventListener('click',event=>{
    document.getElementById('d3Speech')?.classList.add('scan-mode');
    event.currentTarget.disabled=true;event.currentTarget.textContent='SCAN ACTIVE';
  ...
  const originalTerms=['born','birth','conceived','created','founded','formed','established','brought into existence','came into existence','came into being','came to life','came into life','brought to life','foundation','founding','originated','began','began in 1776','founded in 1776'];
  const renewalTerms=['reborn','rebirth','renewed','renewal','revived','restored','start anew','starts anew','begin again','begins again','new beginning','brought back to life','bring back to life','given new life','created anew','bring to life','brought to life','start over','new life','renew the founding ideals','renew founding ideals','restore the founding ideals','restore founding ideals','return to the founding ideals','focus on the founding ideals again','focus on them again','recommit to liberty','recommit to equality','recommit to freedom','recommit to the founding ideals','bring the ideals back to life','fulfil the founding promise','fulfill the founding promise','realise the founding promise','realize the founding promise','preserve the union','rebuild the union','secure the nation s future','secure the country s future'];
  const personificationTerms=['personified','personification','humanised','humanized','treated like a person','treated as a person','depicted as a person','depicted like a person','presented as a person','presented like a person','portrayed as a person','presented as human','depicted as human','human being','compared to a child','presented as a child','like a child','compared to a living being','presented as a living being','depicted as a living being','living being','living entity','something alive','treated as a living entity','given human qualities','given life'];
  ...
  function contextStatus(value){
    ...
    const idealRenewal=hasAny(v,['renew','restore','return','focus','refocus','recommit','revive','bring back','bring to life','preserve','fulfil','fulfill','realise','realize'])&&hasAny(v,['founding ideal','founding principle','original ideal','liberty','equality','freedom','founding promise','country s ideals','nation s ideals','union s ideals']);
    const renewal=hasAny(v,renewalTerms)||idealRenewal;
  ...
  function functionStatus(value){
    const v=norm(value);
    const structure=hasAny(v,['frame','frames','framing','bookend','bookends','beginning and end','beginning with the end','beginning of the speech with the ending','opening and ending','opening with the conclusion','start and end','links the beginning','connects the beginning']);
    const connect=hasAny(v,['connect','link','bridge','frame','bookend']);
    const origin=hasAny(v,['founding','foundation','original birth','birth of the nation','creation of the nation','creation of the union','beginning in 1776','past','founding ideals']);
    const renewal=hasAny(v,['renew','rebirth','new birth','new beginning','future','renewed freedom','responsibility of the living','task of the living','recommit']);
    const survival=hasAny(v,['not fail','not to fail','must not fail','not perish','must not perish','survive','survival','continue to exist','preserve the union','save the union','secure the future','endure','soldiers did not die in vain','not died in vain','country s future','nation s future','goal not to fail']);
    if(connect&&origin&&(renewal||survival)&&structure)return 'correct';
    if(structure||connect&&(origin||renewal||survival))return 'partial';
    return 'retry';
  }

  document.getElementById('showD3FunctionHint')?.addEventListener('click',()=>{
    document.getElementById('d3FunctionHint')?.classList.add('show');
  });

  document.getElementById('verifyD3Function')?.addEventListener('click',()=>{
    const readingY=window.scrollY;
    const status=functionStatus(document.getElementById('d3FunctionInput')?.value);
    const ok=status==='correct';
    document.getElementById('d3FunctionRetry')?.classList.toggle('show',status==='retry');
    document.getElementById('d3FunctionPartial')?.classList.toggle('show',status==='partial');
    document.getElementById('d3FunctionRecovered')?.classList.toggle('show',ok);
    if(!ok)return;
    document.getElementById('d3FunctionHint')?.classList.remove('show');
    const hintButton=document.getElementById('showD3FunctionHint');if(hintButton)hintButton.disabled=true;
  ...
  // D05 · the closing passage remains visible directly after the narrative introduction.

  // D05 · locate the compact closing formula before analysing its overlapping devices.
  const finalPhrases=[...document.querySelectorAll('.final-phrase-select')];
  finalPhrases.forEach(phrase=>phrase.addEventListener('click',()=>phrase.classList.toggle('selected')));
  document.getElementById('verifyD5Formula')?.addEventListener('click',()=>{
    const ok=finalPhrases.length===3&&finalPhrases.every(phrase=>phrase.classList.contains('selected'));
    document.getElementById('d5FormulaRetry')?.classList.toggle('show',!ok);
    document.getElementById('d5FormulaRecovered')?.classList.toggle('show',ok);
    if(!ok)return;
    finalPhrases.forEach(phrase=>{phrase.disabled=true;phrase.classList.remove('selected');phrase.classList.add('mark-final-device');});
    const button=document.getElementById('verifyD5Formula');markVerifyButton(button);if(button)button.disabled=true;
    const analysis=document.getElementById('d5DeviceAnalysis');if(analysis)analysis.hidden=false;
  });
