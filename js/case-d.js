window.initCaseD=function(){
  ...
  // Temporary v41 D review state: earlier device files are pre-recovered so the final D05 recovery
  ...
  function applyDReviewRecoveredState(){
    ...
    const layerLabels={general:'GENERAL EFFECT',context:'MEANING IN CONTEXT',line:'FUNCTION IN THE LINE OF ARGUMENT'};
    document.querySelectorAll('.d1-layer-slot').forEach(slot=>{slot.classList.add('filled');slot.textContent=layerLabels[slot.dataset.layer]||slot.dataset.label;slot.disabled=true;});
    ...
    // D04
    markVerifyButton(document.getElementById('verifyD4Pattern'));
