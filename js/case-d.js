window.initCaseD=function(){
  ...
  document.getElementById('verifyD1Marking')?.addEventListener('click',()=>{
    ...
    const stage=document.getElementById('d1DeviceStage');if(stage)stage.hidden=false;
  });
  ...
  document.getElementById('verifyD1Layers')?.addEventListener('click',()=>{
    ...
    notes.forEach(note=>{note.disabled=true;note.classList.add('verified',`pair-${note.dataset.layer}`);});
  });
  ...
  document.getElementById('verifyD2Parallelism')?.addEventListener('click',()=>{
    ...
    d2Mode='antithesis';
    const stage=document.getElementById('d2AntithesisStage');if(stage)stage.hidden=false;
  ...
  // D04 · explanation remains visible until students explicitly confirm the record.
