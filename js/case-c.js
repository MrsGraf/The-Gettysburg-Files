window.initCaseC=function(){
  ...
  // Temporary review mode remains active in this baseline: C01 is pre-recovered and
  // C03 is treated as already recovered after its archive transmission has been shown.
  // File C itself is NOT pre-restored so the Master Key / C -> D unlock sequence remains testable.
  const REVIEW_MODE_C03_PRECONFIRMED=true;
  ...
  document.getElementById('closeAnalystArchiveAlert')?.addEventListener('click',()=>{
  ...
  analystAlert?.addEventListener('click',event=>{
    if(event.target===analystAlert){
      setArchiveModalOpen(analystAlert,false);
      if(REVIEW_MODE_C03_PRECONFIRMED&&!c03Confirmed){c03Confirmed=true;renderC03State();}
    }
  });
  ...
  // v38 temporary test state: C01 is pre-recovered so C02 can be checked independently.
