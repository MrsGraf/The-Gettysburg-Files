window.initCaseC=function(){
  ...
  function renderSummarySlide(index){
    ...
    if(status){
      status.textContent=summaryVerified[index]?'VERIFIED':'IN PROGRESS';
      status.classList.toggle('verified',summaryVerified[index]);
    }
  ...
  document.getElementById('nextSummarySection')?.addEventListener('click',()=>{
    ...
    const summaryStatus=document.getElementById('summarySlideStatus');
    if(summaryStatus){
      summaryStatus.textContent='RECOVERED';
      summaryStatus.classList.add('verified');
    }
