window.initCaseC=function(){
  ...
  const analystPairs=[
    ...
    },
    {
      ...
      analysis:'Lincoln develops his argument from remembrance toward renewal and continued responsibility.'
    }
  ];
  ...
  document.getElementById('verifyArgumentFunctions')?.addEventListener('click',()=>{
    ...
    if(ok){
      markVerifyButton(document.getElementById('verifyArgumentFunctions'));
  ...
  document.getElementById('verifyTimeStructure')?.addEventListener('click',()=>{
    ...
    const spellingCorrections=targets.filter((target,i)=>ok&&values[i]!==target&&closeEnough(values[i],target));
    ...
    if(ok){
      ...
      if(spellingCorrections.length)spellingAccepted(spellingCorrections.map(x=>x.toUpperCase()).join(' · '));
      setTimeout(()=>showRecoveryOverlay({caseId:'C',fragment:'27',nextId:'O0',nextLabel:'RETURN TO ARCHIVE'}),spellingCorrections.length?3100:450);
    }
  });
};