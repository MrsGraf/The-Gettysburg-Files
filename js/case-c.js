window.initCaseC=function(){
  ...
  const summaryConcepts=[
    ['founding','founded','found','1776','liberty','equality','equal','creation','created','new nation','ideals','past'],
    ['civil war','war','test','tested','testing','survive','survival','endure','crisis','nation','present'],
    ['gettysburg','battlefield','cemetery','dedication','dedicate','gathering','gather','occasion','meeting','present'],
    ['soldier','soldiers','sacrifice','words','ceremony','dedication','dedicate','consecrate','hallow','deeds','what they did','limits'],
    ['living','responsibility','unfinished work','unfinished','future','task','freedom','renewal','government','action','continue','devotion']
  ];
  const strongPhrases=[
    ['founding of the nation','founding ideals','liberty and equality'],
    ['civil war','nation being tested','test of whether the nation'],
    ['gathering at gettysburg','dedication of the cemetery','battlefield at gettysburg'],
    ["soldiers' sacrifice",'soldiers sacrifice','limits of words','what the soldiers have done'],
    ['responsibility of the living','unfinished work','future action','new birth of freedom']
  ];

  function summaryMatches(value,index){
    const v=normalizeText(value);
    if(v.length<5)return false;
    if(strongPhrases[index].some(phrase=>v.includes(phrase)))return true;
    const hits=summaryConcepts[index].filter(term=>v.includes(term)).length;
    return hits>=2;
  }
  ...
  functionTokens.forEach(token=>token.addEventListener('click',()=>{
    if(token.classList.contains('assigned'))return;
    functionTokens.forEach(x=>x.classList.remove('active'));
    token.classList.add('active');
    selectedFunction=token;
  }));

  function updateFunctionVerify(){
    const button=document.getElementById('verifySectionFunctions');
    if(button)button.disabled=!functionSlots.every(slot=>slot.classList.contains('filled'));
  }
  ...
  functionSlots.forEach(slot=>slot.addEventListener('click',()=>{
    if(slot.classList.contains('filled')&&!selectedFunction){
      const oldValue=slot.dataset.assignedValue;
      functionTokens.find(token=>token.textContent===oldValue)?.classList.remove('assigned');
    ...
    if(!selectedFunction)return toast('Select a function card first.');
    if(slot.classList.contains('filled')){
      const oldValue=slot.dataset.assignedValue;
      functionTokens.find(token=>token.textContent===oldValue)?.classList.remove('assigned');
    }
    setSlotValue(slot,selectedFunction.textContent);
    slot.classList.add('filled');
    slot.dataset.assignedValue=selectedFunction.textContent;
    slot.dataset.assignedSec=selectedFunction.dataset.sec;
    selectedFunction.classList.remove('active');
    selectedFunction.classList.add('assigned');
  ...
  document.getElementById('verifySectionFunctions')?.addEventListener('click',()=>{
    const ok=functionSlots.every(slot=>slot.dataset.sec===slot.dataset.assignedSec);
    document.getElementById('c2retry')?.classList.toggle('show',!ok);
    document.getElementById('c2fb')?.classList.toggle('show',ok);
    const macro=document.getElementById('timeStructureTask');
    if(macro)macro.hidden=!ok;
    if(ok){
      markVerifyButton(document.getElementById('verifySectionFunctions'));
      functionTokens.forEach(token=>token.disabled=true);
      functionSlots.forEach(slot=>slot.disabled=true);
    }
  });
