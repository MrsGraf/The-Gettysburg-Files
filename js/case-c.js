window.initCaseC=function(){
  ...
  // Temporary File-C test state in this baseline: only C01 is pre-recovered.
  // C02, the external C03 confirmation, and File C itself must be completed normally.
  const REVIEW_MODE_C03_PRECONFIRMED=false;
  ...
  function showAnalystTransmission(){
  ...
  function handleAnalystRecordDownload(){
    ...
    // Return to the C02 workspace after the download action so the external-C03
    // status and confirmation controls are immediately accessible.
  ...
  document.getElementById('reopenAnalystTransmission')?.addEventListener('click',showAnalystTransmission);
  ...
  document.getElementById('closeAnalystArchiveAlert')?.addEventListener('click',closeAnalystTransmission);
  document.getElementById('closeAnalystTransmission')?.addEventListener('click',closeAnalystTransmission);
  analystAlert?.addEventListener('click',event=>{
    if(event.target===analystAlert)closeAnalystTransmission();
  });
  document.addEventListener('keydown',event=>{
    if(event.key==='Escape' && analystAlert?.classList.contains('show'))closeAnalystTransmission();
  });
