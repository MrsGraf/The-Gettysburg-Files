window.GettysburgState=(()=>{
  const KEY='gettysburgFragmentsV41Test';
  const SNAPSHOT_KEY='gettysburgCaseSnapshotsV41Test';
  let fragments=['18','63','',''];
  let snapshots={};
  ...
  // v41 temporary review baseline: A/B remain restored; C02 and File C stay open; D remains unrecovered so its final recovery/epilogue path can be tested.
  // New versioned storage keys and these guards prevent stale completion data from skipping the intended test states.
  fragments[2]='';
  fragments[3]='';
  delete snapshots.C;
  delete snapshots.D;
