window.GettysburgState=(()=>{
  const KEY='gettysburgFragmentsV43LastOnlyTest';
  const SNAPSHOT_KEY='gettysburgCaseSnapshotsV43LastOnlyTest';
  let fragments=['','','',''];
  let snapshots={};
  ...
  // v43 temporary last-step review baseline: all case files remain unrecovered,
  // but the archive unlocks all four overview cards so only the final step inside
  // each file needs to be tested. Reset the state on every fresh load.
  fragments=['','','',''];
  snapshots={};
