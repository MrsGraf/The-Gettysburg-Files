...
const FILES=['prologue','archive-overview','case-a','case-b','case-c','case-d','final-key'];
// Test-only access is disabled in the regular classroom baseline.
...
function initInstructionGates(){
  document.querySelectorAll('.screen[data-case]').forEach(screen=>{
    ...
    while(node){
      ...
      // Preserve stages that are intentionally hidden by the file workflow.
      // The briefing gate may temporarily hide already-visible content, but it
      // must never unlock a later reconstruction stage on its own.
      if(!node.hidden){
...
function updateProgress(screen){
  ...
  else if(screen.dataset.id==='K0') overall=100;
  ...
  if(keyMini)keyMini.hidden=['prologue','K0'].includes(screen.dataset.id)||(screen.dataset.id==='O0'&&finalKeyComplete);
  renderKey();
...
function renderArchiveOverview(){
  ...
  const archiveRestored=complete&&finalArchiveOverlayCompleted;
  masterCard?.classList.toggle('locked',!complete);
  masterCard?.classList.toggle('available',complete&&!archiveRestored);
  masterCard?.classList.toggle('restored',archiveRestored);
  ...
  if(masterLockText)masterLockText.textContent=archiveRestored?'FINAL ARCHIVE RESTORATION · COMPLETE':complete?'FINAL ARCHIVE RESTORATION · READY':'FINAL ARCHIVE RESTORATION · INCOMPLETE';
  if(masterButton){
    masterButton.disabled=!complete;
    masterButton.textContent=archiveRestored?'OPEN FINAL BRIEFING':complete?'OPEN FINAL RESTORATION':'LOCKED';
    masterButton.classList.toggle('ready-action',complete&&!archiveRestored);
    masterButton.classList.toggle('completed-action',archiveRestored);
    masterButton.classList.toggle('primary',archiveRestored);
    masterButton.classList.toggle('ghost',!complete);
  }
  if(masterText)masterText.textContent=archiveRestored
    ? 'Archive Case #272 is fully restored. The final Analyst Briefing remains available here, and all four restored files can be reopened for review.'
    : count===4
...
function showRecoveryOverlay({caseId,fragment,nextId='O0',nextLabel='RETURN TO ARCHIVE'}){
  ...
  revealNote.textContent='Note this code fragment for the Master Recovery Key. You can reopen it later from the restored file if needed.';
...
function initFinalArchiveOverlay(){
  ...
  const masterCard=document.getElementById('masterKeyDirectoryCard');
  const openRecovery=()=>{
    if(GettysburgState.getFragments().every(Boolean)){
      if(finalArchiveOverlayCompleted)showFinalArchiveCompleteOverlay();
      else showFinalArchiveKeyOverlay();
    }
  ...
  document.getElementById('returnToRestoredFilesFromComplete')?.addEventListener('click',()=>{
    const overlay=document.getElementById('finalArchiveOverlay');
    overlay?.classList.remove('show');
    overlay?.setAttribute('aria-hidden','true');
    document.body.classList.remove('final-archive-overlay-open');
...
function showFinalArchiveCompleteOverlay(){
  const overlay=document.getElementById('finalArchiveOverlay');
  if(!overlay)return;
