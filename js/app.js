...
// Temporary v41 review mode: all archive files and the Master Recovery Key are accessible for testing.
// Internal task completion is still controlled separately (notably C02 remains open).
...
function renderArchiveOverview(){
  ...
  const complete=REVIEW_UNLOCK_ALL||count===4;
  ...
  if(masterText)masterText.textContent=count===4
    ...
    : (REVIEW_UNLOCK_ALL?'Review mode: final archive access is temporarily unlocked for testing.':'Recover all four case files to unlock the Master Recovery Key.');
}
