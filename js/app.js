...
function initInstructionGates(){
  document.querySelectorAll('.screen[data-case]').forEach(screen=>{
    ...
    transmission.className='archive-transmission-label';
    transmission.innerHTML='<span class="transmission-signal" aria-hidden="true"><i></i><i></i><i></i></span><span>ARCHIVE TRANSMISSION</span>';
    intro.prepend(transmission);

    const content=document.createElement('div');
    content.className='instruction-gated-content';
    content.hidden=true;
    while(intro.nextSibling)content.appendChild(intro.nextSibling);
